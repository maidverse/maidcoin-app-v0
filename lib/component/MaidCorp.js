...
class MaidCorp extends skynode_1.DomNode {
    constructor() {
        super(".maid-corp");
        this.append(skynode_1.el("h4", "Maid Corporation"), this.control = skynode_1.el(".control", skynode_1.el("a.deposit-button", "Deposit", {
            click: async () => {
    ...
    async load() {
        ...
        if (owner !== undefined) {
            skynode_1.el("a.claim-button", `Claim ${ethers_1.utils.formatEther(await TheMasterContract_1.default.getPendingReward(1, owner))} $MAID`, {
                click: async () => {
