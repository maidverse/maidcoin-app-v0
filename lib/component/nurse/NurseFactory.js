...
class NurseFactory extends skynode_1.DomNode {
    ...
    async load() {
        ...
        if (owner !== undefined) {
            ...
            this.empty().append(skynode_1.el("img.image", { src: `https://storage.googleapis.com/maidcoin/NursePart/${this.nurseType}.png` }), skynode_1.el(".info", skynode_1.el(".property", `Nurse Part #${this.nurseType}`), skynode_1.el(".property", `Balance: ${balance}`), skynode_1.el(".property", `Need Part Count: ${nurseType.partCount}`), skynode_1.el(".control", skynode_1.el("a.assemble-button", "Assemble Nurse", {
                click: async () => {
