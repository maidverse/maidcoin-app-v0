...
class MaidSummary extends skynode_1.DomNode {
    ...
    set maid(maid) {
        this.empty().append(skynode_1.el("", `Maid #${this.maidId}`), skynode_1.el("", `Origin Power: ${maid.originPower}`), skynode_1.el("", `Supported LP Token Amount: ${maid.supportedLPTokenAmount}`), skynode_1.el("a.support-button", "Support", {
            click: async () => {
