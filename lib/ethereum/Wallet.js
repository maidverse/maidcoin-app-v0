...
class Wallet extends eventcontainer_1.default {
    ...
    async signTypedData(owner, deadline, name, version, verifyingContract, Permit, message) {
        var _a;
        const EIP712Domain = [
    ...
    async signERC721PermitAll(name, version, verifyingContract, spender, nonce, timelimit) {
        const owner = await this.loadAddress();
